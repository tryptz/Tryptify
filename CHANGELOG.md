...
- **Apple Music is gone from Settings** — the instance URL, the Tailscale wrapper address and its secret, the format picker, the Dolby Atmos toggle and the "Apple only" catalog option are all removed from the Instances tab. The integration itself is untouched: playback, downloads and search still work, and the stored values are still read, so nothing that was already set up stops working — there is just no longer a screen for it. Anyone left on "Apple only" now reads as "Both", because the picker can no longer show that state and you would have had no way to see it or get out of it.
- **Apple Music results keep loading as you scroll** — search fetched Apple for page 1 and then never again: the paging code had TIDAL and Qobuz branches and no Apple one, so scrolling silently dropped Apple from every page after the first. In Apple-only mode it was worse — TIDAL and Qobuz are both marked exhausted at page 1 there, so paging considered itself finished and infinite scroll fetched nothing at all, leaving you with one page of results and no way to see more.
