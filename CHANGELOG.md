...
#### Radio Queue Maker
- **Radio (queue maker)** — new `RadioQueueManager` seeds a station from the playing track, asks the optional Tryptify-Playlist planner (`POST /api/radio/plan`) for query/candidate hints, resolves them to real catalog tracks (planner hints → TIDAL recommendations → query search), dedupes against the queue/history/session by id and normalized artist|title, and appends batches through `QueueManager` with automatic refill near the queue tail. The planner is strictly advisory — radio works fully offline from catalog recommendations, and a queue reset stops radio so it can't instantly refill the tail the user just rejected.
