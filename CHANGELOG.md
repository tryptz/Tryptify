...
- **Bass-reactive lyrics (Nightcore-style)** — the active line now reacts to the music in real time from the existing `SpectrumAnalyzerTap` FFT (40–110 Hz bins → dB level → ~12 ms attack / 150 ms release envelope → underdamped spring, ζ≈0.38, so kicks overshoot ~25% and ring once — bounce, not jitter). The pulse drives a scale pump (up to +8%), a soft accent-colored radial glow, and 8 slowly-rotating god rays whose length/brightness/thickness ride the bass; each new line pops in with its own 0→1 spring right on the beat that activated it. Everything is draw-phase only (zero per-frame recomposition), the analyzer stake is ref-counted to the lyrics being on screen, and a new "Bass Reaction" slider in Lyrics Appearance scales the whole treatment (0 disables it and never touches the audio engine).
