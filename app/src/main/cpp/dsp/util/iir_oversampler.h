...
// The Butterworth oversampler ChannelOversampler replaced, kept for the one
// effect that oversamples inside itself: the Ladder Filter runs its feedback
// loop at 2x for stability, per sample, and needs no latency there — a delay
// inside a feedback loop would retune it. Minimum phase, so zero latency, and
// its phase shift is part of the ladder's sound as it always was.
//
// zero-stuff + 8th-order Butterworth anti-image filter on the way up, matching
// anti-alias filter + decimation on the way down, both at 0.9 x the base
// Nyquist. 4x is two cascaded 2x stages.
