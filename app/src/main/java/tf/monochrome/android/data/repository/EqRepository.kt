...
,
    ...
    suspend fun savePreset(preset: EqPreset) {
        ...
        // Queued on save rather than left for the Sync button, so a profile
        // tuned on the phone is already in the account by the time the tablet
        // next opens — and held until the cloud accepts it, so an offline save
        // still gets there.
    ...
    suspend fun deletePreset(presetId: String) {
        ...
        // Deleting here deletes it everywhere. The delete is held until the
        // cloud accepts it, and the pull skips the preset meanwhile — a delete
        // made offline used to be lost, and the preset came back on the next
        // pull.
